 ...
 * Code used below from the Nginx project. Some code Copyright (C) Jordan Henderson.
 */
...
ngx_int_t boot_nginx(int argc, char** argv) {
		ngx_int_t         i;
		ngx_log_t        *log;
...
ngx_add_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    for (p = inherited, v = p; *p; p++) {
        if (*p == ':' || *p == ';') {
            ...
            ls = (ngx_listening_t*) ngx_array_push(&cycle->listening);
            if (ls == NULL) {
                return NGX_ERROR;
            }
...
tz_found:
    ...
    if (last) {
        ...
        *last = n;
        ...
        env = (char**) ngx_palloc(cycle->pool, (n + 1) * sizeof(char *));
    }
...
ngx_exec_new_binary(ngx_cycle_t *cycle, char *const *argv)
{
    ...
    var = (char*) ngx_alloc(sizeof(NGINX_VAR)
...
ngx_core_module_create_conf(ngx_cycle_t *cycle)
{
    ...
    ccf = (ngx_core_conf_t*) ngx_pcalloc(cycle->pool, sizeof(ngx_core_conf_t));
    if (ccf == NULL) {
        return NULL;
    }
